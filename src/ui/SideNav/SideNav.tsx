...
interface SideNavProps {
  readonly pxFromWhichStaticBehaviour: number,
}

export default function SideNav({ pxFromWhichStaticBehaviour }: SideNavProps) {
  ...
  useLayoutEffect(calcNavPanelMetrics, [calcNavPanelMetrics]);
  ...
  const burgetBtnOnClick = useCallback<MouseEventHandler<HTMLButtonElement>>(() => {
      ...
      <>
        ...
        <nav
          className={`fixed top-0 left-0 min-w-[220px] w-[14.58%] h-full 
            border-solid border-r-[1px] border-grey-200 px-[1.3%] py-[32px]
  ...
  return (
    <>
      <span
        className="w-full"
        style={{
          height: `${navPanelMetrics.height || '55'}px`,
        }}
      ...
      {windowMetrics.width !== 0 && (
        <>
          ...
          >
            ...
            >
              <ul className="flex flex-col justify-start items-stretch gap-[16px]">
                <li>
                  <NavLink text="Home" href={links.dashboard} />
                </li>
                <li>
